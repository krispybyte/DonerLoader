...
namespace Utilities
{
    ...
    std::tuple<std::string, std::string> EncryptMessage(const std::string& Plain, CryptoPP::SecByteBlock& AesKey);
    // Wrapper for aes decryption and hex decoding.
    std::string DecryptMessage(const std::string& EncodedCipher, const std::string& EncodedIv, CryptoPP::SecByteBlock& AesKey);
